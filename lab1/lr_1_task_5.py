...
Original file is located at
    https://colab.research.google.com/drive/1xpLA-bzkZA98eLLxcRQgIoDNzmKkzv9x
"""
...
assert np.array_equal(diachenko_confusion_matrix(df.actual_label.values, df.predicted_RF.values), confusion_matrix(df.actual_label.values, df.predicted_RF.values) ), 'my_confusion_matrix() is not correct for RF'
assert  np.array_equal(diachenko_confusion_matrix(df.actual_label.values, df.predicted_LR.values),confusion_matrix(df.actual_label.values, df.predicted_LR.values) ), 'my_confusion_matrix() is not correct for LR'
...
def diachenko_accuracy_score(y_true, y_pred):
    ...
    return accuracy
assert diachenko_accuracy_score(df.actual_label.values, df.predicted_LR.values) == accuracy_score(df.actual_label.values, df.predicted_LR.values), 'my_accuracy_score failed on LR'
...
def diachenko_recall_score(y_true, y_pred):
    ...
    accuracy = (TP) / (TP  + FN)
    return accuracy
assert diachenko_recall_score(df.actual_label.values, df.predicted_LR.values) == recall_score(df.actual_label.values, df.predicted_LR.values), 'my_accuracy_score failed on LR'
...
def diachenko_precision_score(y_true, y_pred):
    ...
    return accuracy
assert diachenko_precision_score(df.actual_label.values, df.predicted_LR.values) == precision_score(df.actual_label.values, df.predicted_LR.values), 'my_accuracy_score failed on LR'
...
def diachenko_f1_score(y_true, y_pred):
    ...
    return f1
assert diachenko_f1_score(df.actual_label.values, df.predicted_LR.values) == f1_score(df.actual_label.values, df.predicted_LR.values), 'my_accuracy_score failed on LR'
...
print('')
print('scores with threshold = 0.25')
print('Accuracy LR: %.3f'%(diachenko_accuracy_score(df.actual_label.values, (df.model_LR >= 0.25).astype('int').values)))
print('Recall LR: %.3f'%(diachenko_recall_score(df.actual_label.values, (df.model_LR >= 0.25).astype('int').values)))
print('Precision LR: %.3f'%(diachenko_precision_score(df.actual_label.values, (df.model_LR >= 0.25).astype('int').values)))
print('F1 LR: %.3f'%(diachenko_f1_score(df.actual_label.values, (df.model_LR >= 0.25).astype('int').values)))
